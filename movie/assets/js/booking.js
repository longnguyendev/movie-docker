...
$(document).ready(function () {
  ...
  function createSeat(seatRow, seatStart) {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      seatNumber[seatNumber.length - 1].innerHTML += `
                 <a class="seat-number ${seatVip}" href="#seat-booking" class="seat">${seatRow}${
        i + 1 + seatStart
      }</a>
             `;
    }
  ...
  const currentPrice = document.querySelector("#seat-booking");
  const selected = $("#seat-selected");
  const price = $("#total-price");
  var seat = $(".seat-number");
  const delAll = $("#del-all");
  // Money count
  let normalPrice = Number(currentPrice.dataset.price);
  let vipPrice = Math.ceil(normalPrice * (100 / 80));
  let money = 0;
  for (let i = 0; i < 30; i++) {
  ...
  seat.click(function () {
    if (!$(this).hasClass("select")) {
      ...
      price.text(`${money.toLocaleString("vi-VN")}đ`);
    } else {
      if ($(this).hasClass("vip")) {
        money -= vipPrice;
      } else {
