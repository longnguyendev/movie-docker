...
class BillModel extends Model
{
    ...
    {
        $sql = parent::$connection->prepare('SELECT created_at, user.name, product.name, seats, address, timePay, totalPrice FROM user INNER JOIN bill ON bill.user_id = ? INNER JOIN product on product.id = bill.product_id ORDER BY created_at DESC');
        $sql->bind_param("s", $id);
        $items = [];
        $sql->execute();
